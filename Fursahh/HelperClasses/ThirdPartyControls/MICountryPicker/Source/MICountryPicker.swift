...
open class MICountryPicker: UITableViewController {
    ...
    fileprivate func createBackButton() {
        if var image = UIImage(named: "icon_btn_back") {
            
            if Locale.current.languageCode == "ar" {
                image = image.imageFlippedForRightToLeftLayoutDirection()
            }            
            
            let backItem = UIBarButtonItem(image: image.withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(backWithPop))
            self.navigationItem.leftBarButtonItem = backItem
        } else {
            let backItem = UIBarButtonItem(title: "Cancel", style: .done, target: self, action: #selector(backWithPop))
            self.navigationItem.leftBarButtonItem = backItem
        }
        
        //let image = #imageLiteral(resourceName: "icon_btn_back").withRenderingMode(.alwaysOriginal)
    }
    ...
    fileprivate func filter(_ searchText: String) -> [MICountry] {
        ...
        sections.forEach { (section) -> () in
            section.countries.forEach({ (country) -> () in
                if country.name.length >= searchText.length {
